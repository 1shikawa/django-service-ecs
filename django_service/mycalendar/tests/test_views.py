...
class ScheduleTest(TestCase):
    ...
    def test_get_schedule_nodata(self):
        """
        schedule登録データがあることを検証
        """
        schedule_data = Schedule.objects.all().count()
        self.assertNotEqual(schedule_data, 0)
    ...
    def test_get_navlink(self):
        ...
        self.assertTemplateUsed(response, 'calendar/month_with_schedule.html')

        response = self.client.get('/DailyInputList/')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendar/DailyInputList.html')

        response = self.client.get('/DailySumList/')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendar/DailySumList.html')

        response = self.client.get('/MonthlySumList/')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendar/MonthlySumList.html')
    ...
    def test_get_schedule(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendar/multiAdd.html')

        response = self.client.get('/month_with_schedule/NewMultiEdit/2018/12/1')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'calendar/multiEdit.html')
